...
const client = postgres(connectionString, { prepare: false, ssl: "require", max: 1 });
const db = drizzle(client, { schema });

// Using the same seed data from mock-data/dashboard.ts
const projectSeeds = [
...
async function seed() {
  ...
  const insertedUsers = await db.insert(schema.users).values(
    projectSeeds.map((p) => ({
      name: p.ownerName,
      email: `${p.ownerAvatarSeed}@example.com`,
      avatarSeed: p.ownerAvatarSeed,
    }))
  ).returning();
  ...
  await db.insert(schema.tasks).values(
    todayTasks.map((t) => {
      // Find matching project by color/name roughly or just use the first project
      const matchIndex = projectSeeds.findIndex(p => p.name === t.projectName);
      const projectId = matchIndex !== -1 ? insertedProjects[matchIndex].id : insertedProjects[0].id;
      return {
        name: t.name,
        projectId,
  ...
  await db.insert(schema.finance).values([
    { amount: "5000.00", type: "income", category: "Services", date: new Date() },
    { amount: "1200.00", type: "expense", category: "Software", date: new Date() },
    { amount: "3000.00", type: "income", category: "Consulting", date: new Date() },
    { amount: "800.00", type: "expense", category: "Marketing", date: new Date() },
  ]);
